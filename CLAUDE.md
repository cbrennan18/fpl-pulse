...
**Domain migration in progress: cbrennan.ie → ciaranbrennan.ie. Old domain expires 21 Oct 2026.**

The repo's Pages custom domain field is **empty** — the project site inherits the apex from the user site (`cbrennan18.github.io`). The `postbuild` script writes `dist/CNAME` anyway and the file IS present on `origin/gh-pages`, but GitHub ignores it: a project repo cannot claim a domain the user site already holds. Two-step plan, do not shortcut: (1) migration night — change the echo to `ciaranbrennan.ie`, conservative, keeps current behaviour; (2) post-migration sweep — delete the echo leaving `touch dist/.nojekyll`, deploy and verify. If the site still serves, the write was inert as inferred; if it 404s, restore it pointing at ciaranbrennan.ie.
