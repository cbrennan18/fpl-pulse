...
FPL Pulse frontend — a React SPA for tracking Fantasy Premier League mini-leagues. Deployed to GitHub Pages at `ciaranbrennan.ie/fpl-pulse`. The backend is a Cloudflare Worker (`../fpl-pulse-worker/`) with its own repo and CLAUDE.md.
...
Static SPA on GitHub Pages under `base: /fpl-pulse/`. Deploy with `npm run deploy` (gh-pages → `dist`).

**Domain migrated cbrennan.ie → ciaranbrennan.ie on 4 Sep 2026.** A 301 is live on the old domain, which **expires 21 Oct 2026** — after that any surviving `cbrennan.ie` URL is permanently dead, not redirected. The repo sweep is done (`index.html` OG/canonical meta, `package.json` `homepage`, the awards-PNG footer in `GwAwardsGraphic.jsx`, and the docs). Two deliberate holdovers remain, both waiting on 21 Oct: the `postbuild` CNAME step below, and the `cbrennan.ie` entries in `PROD_HOSTS` (see Analytics).

The repo's Pages custom domain field is **empty** — the project site inherits the apex from the user site (`cbrennan18.github.io`). The `postbuild` script writes `dist/CNAME` anyway and the file IS present on `origin/gh-pages`, but GitHub ignores it: a project repo cannot claim a domain the user site already holds. **Step 1 is done** — the echo now writes `ciaranbrennan.ie`, conservative, keeping the existing behaviour. **Step 2 is the only outstanding piece:** delete the echo entirely, leaving `postbuild` as just `touch dist/.nojekyll`, then `npm run deploy` and confirm FPL Pulse still serves at `ciaranbrennan.ie/fpl-pulse`. If it does, the CNAME write was inert as inferred; if it 404s, restore the echo pointing at `ciaranbrennan.ie`. Do not shortcut step 2 by bundling it with other deploys — it needs a clean before/after.

- **Deep-link SPA shim — LOAD-BEARING, do not naively edit.** GH Pages 404s any deep path on a cold load (e.g. a shared `/fpl-pulse/wrapped?league=X&via=link`). `public/404.html` encodes the full path + query into a single `?/…` param and redirects to the app root; an inline decode script at the **top of `index.html` `<head>`** (before the deferred bundle) restores the real URL via `history.replaceState` so BrowserRouter boots on the correct route. Standard rafgraph/spa-github-pages pair, `pathSegmentsToKeep = 1` (keeps the `/fpl-pulse/` segment; 0 double-prepends the basename, 2 leaves a still-404ing target). If you revert `404.html` to a plain redirect, **every shared link 404s to Landing** — the two files are a matched pair. **Migration note:** old shared links will take four hops (301 → new domain → GH Pages 404 → encode → decode → route). The redirect's query-string preservation and the shim's encoding must compose correctly; this is explicitly tested on migration night, not assumed.
- **OG / unfurl meta is STATIC in `index.html`** (wrapped-framed), one shell for the whole app — no per-route/per-league variation, because unfurl crawlers don't run JS (so meta must **never** be react-helmet / JS-injected). `public/og-wrapped.png` (declared 1200×630, shipped 2× retina) is the share card; hrefs are literal absolute `ciaranbrennan.ie` URLs (lines 30, 38, 42, 49), migrated 4 Sep 2026. Per-league dynamic OG would need worker head-injection (deferred).
- **Analytics:** `hooks/useUmami.js` — self-hosted Umami, **prod-gated by hostname** (nothing fires in dev/preview), exposes `track(name, props)`. House style is to fire events **inline at the interaction/transition point** (see `league/`, `landing/`, and the wrapped funnel in `WrappedContainer`); no analytics wrapper/context. Endpoint cut over to `analytics.ciaranbrennan.ie` on 3 Sep 2026. `PROD_HOSTS` still lists all four hosts (both domains, apex and www). The `cbrennan.ie` entries are **not** load-bearing for redirected traffic — the 301 resolves before any JS runs, so `window.location.hostname` is always the new domain. They stay only because the old collector at `analytics.cbrennan.ie` is still live and is being retired separately, after a week of confirming the new endpoint records cleanly; trim them then.
